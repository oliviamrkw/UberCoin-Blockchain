...
while True:
    ...
    if id_input.isdigit() and len(id_input) == 1 and id_input != '0':
        ...
        if id not in id_list: # checks if id is in the list already, adds the id if not and adds the number of ubercoins (calculated from the distance) as the value
            add_id_to_list(id, id_list)
        add_ubercoins_to_id(id, calculate_ubercoins(distance), id_list)

        blockchain.add_block(id, distance)
